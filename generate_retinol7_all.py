...
BASE    = Path("/Users/lunahansen/Desktop/Claude ads/retinol_base_7.jpg")
OUT_DIR = Path("/Users/lunahansen/Desktop/Claude ads/output/retinol_7")
...
for r in REVIEWS:
    ...
    wrapped_hl = wrap_text(draw, combined, font_hl, MAX_W_HL)
    lines = wrapped_hl.split('\n')

    # Pre-calculate line metrics
    line_metrics = []
    for line in lines:
        lbbox = draw.textbbox((0, 0), line, font=font_hl, anchor="lt")
        line_metrics.append((lbbox[2] - lbbox[0], lbbox[3] - lbbox[1]))

    # First pass: draw per-line black backgrounds (overlap so they merge)
    y = HL_TOP
